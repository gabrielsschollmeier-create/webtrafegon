...
import {
  ...
  Calendar, BarChart2, Settings, Webhook, ChevronRight, Flame,
  FolderOpen, Package, Users2, Zap, Shield, BookOpen, MessageCircle, Home, LayoutGrid
} from 'lucide-react'
...
function NavItem({ to, icon: Icon, label, delay = 0, end: endProp }) {
  return (
    <motion.div
      ...
      >
        {({ isActive }) => (
          <>
            ...
            {isActive && (
              <ChevronRight size={12} className="text-accent/60 relative z-10" />
            )}
          </>
        )}
      </NavLink>
...
export default function Sidebar({ user }) {
  ...
  return (
    <aside
      ...
      style={{ background: '#12141e', boxShadow: '1px 0 0 rgba(255,255,255,0.06)' }}
    >
      {/* Logo */}
      <motion.div
      ...
      >
        ...
        <div className="mt-2 px-0.5">
          <p className="text-[10px] leading-snug tracking-wide" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Acelerar negócios &
          </p>
          <p className="text-[10px] leading-snug font-bold tracking-wide text-accent">
      ...
      <motion.div
        ...
        className="mx-3 mt-3 mb-1 px-3 py-1.5 rounded-xl flex items-center gap-2"
      ...
      <nav className="flex-1 px-3 py-3 overflow-y-auto space-y-0.5">
        ...
        {navCRM.map((item, i) => (
          <NavItem key={item.to} {...item} delay={0.04 + i * 0.03} />
        ))}
        ...
        {navERP.map((item, i) => (
          <NavItem key={item.to} {...item} delay={0.16 + i * 0.03} end={false} />
        ))}
      </nav>
      ...
      >
        {navBottom.map((item, i) => (
          <NavItem key={item.to} {...item} delay={0.04 * i} />
        ))}

        {user && (
          <motion.div
            whileHover={{ backgroundColor: 'rgba(255,255,255,0.05)' }}
            className="flex items-center gap-3 px-3 py-2.5 mt-1 rounded-xl cursor-pointer transition-colors"
          >
